...
const ProfileScreen = () => {
  ...
  const [blockPeople, setBlockPeople] = useState([])
  ...
  const logout = async () => {
    try {
      await AsyncStorage.removeItem('hinge_token').then((res) => {
        console.log('removeToken error', res)
        dispatch(removeToken())
        navigation.navigate('AuthStack', { screen: 'PreLogin' })
      })
    } catch (err) {
  ...
  const fetchUser = async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        setBlockPeople(res.data.user.blockProfiles)

        // console.log('res123', res.data.user.blockProfiles)
        // console.log('res123', res.data.user.saveProfiles)
      })
  ...
  const getOrder = async () => {
    try {
      await axios
        .post(`${baseUrl}/api/user/get-order`, {
          userId,
        })
        .then((res) => {
  ...
  const deleteId = async (userId) => {
    try {
      await axios
        .post(`${baseUrl}/api/user/delete-id`, {
          userId,
        })
        .then((res) => {
          if (res.status == 200) {
      ...
      >
        ...
        >
          ...
          >
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate('BlockPeople', { blockPeople })
              }}
              style={{
                borderWidth: 1,
                borderColor: 'gray',
          ...
          >
            ...
            >
              {rule === false ? (
                <TouchableOpacity onPress={() => setRule(true)}>
                  <AntDesign name="down" size={24} color="black" />
                </TouchableOpacity>
              ) : (
                <TouchableOpacity onPress={() => setRule(false)}>
          ...
          {rule === true ? (
            <View>
              <TouchableOpacity onPress={() => navigation.navigate('Prom1')}>
                <Text>이용약관 내용확인(클릭)</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => navigation.navigate('Prom2')}>
                <Text>개인정보수집 내용확인(클릭)</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => navigation.navigate('Prom3')}>
                <Text>개인 위치정보 이용 내용확인(클릭)</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => navigation.navigate('Prom4')}>
                <Text>가상아이템 정책 내용확인(클릭)</Text>
              </TouchableOpacity>
            </View>
          ) : null}
        </View>
        ...
        >
          ...
          >
            <View style={{ borderBottomWidth: 1, padding: 3 }}>
              <Text style={{ fontSize: 20 }}>정말 계정을 삭제하겠습니까?</Text>
            </View>
            <View style={{ marginTop: 30 }}>
              <Text style={{ color: 'gray' }}>계정을 삭제하시면 </Text>
              <Text style={{ color: 'gray' }}>나혼자 솔로에 등록된</Text>
              <Text style={{ color: 'gray' }}>
                본인의 모든 정보가 삭제됩니다.
              </Text>
            </View>
            <View
              style={{ flexDirection: 'row', marginTop: 30, borderTopWidth: 1 }}
            >
              <TouchableOpacity
                onPress={() => deleteId(userId)}
                style={{
                  marginTop: 20,
                  borderColor: 'gray',
                  borderRadius: 15,
              ...
              >
                <Text style={{ fontSize: 30, color: 'white' }}>삭제</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={{ fontSize: 30 }}>취소</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
              ...
              {order &&
                order?.map((o, index) => (
                  ...
                  >
                    ...
                    <View>
                      <Text>하트{o.count}개</Text>
                    </View>
                    <View>
                      <Text>{o.timeData}</Text>
                    </View>
                  </View>
                ))}
              {order == '' && (
                <View>
                  <Text>하트 구입내역이 없습니다.</Text>
                </View>
              )}
              <TouchableOpacity
