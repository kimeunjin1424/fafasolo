...
const ChargeHeart = () => {
  ...
  const createOrder = async (count) => {
    try {
      await axios
        .post(`${baseUrl}/api/user/create-order`, {
          userId: userId,
          count: count,
          price: count == 10 ? 1500 : count == 25 ? 3000 : 4000,
        })
        .then((res) => {
          console.log(res.status)
        })
        .catch((err) => {
          console.log('charge heart error', err)
        })
    } catch (error) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center' }}>
      ...
      <TouchableOpacity
        activeOpacity={0.8}
        style={{ marginTop: 30, justifyContent: 'flex-end', marginLeft: 30 }}
      >
