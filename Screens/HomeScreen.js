...
const HomeScreen = () => {
  ...
  const { token, userId, user, likedPeople, savePeople, blockPeople } =
  ...
  useEffect(() => {
    LogBox.ignoreLogs(['ReactImageView: Image source "null" does'])
  }, [])

  useEffect(() => {
    if (user.blacklist == 'yes') navigation.navigate('BlackScreen')
  }, [])
  ...
  useEffect(() => {
    const getApiUsers = async () => {
      // console.log(
      //   'blockuserId',
      //   user?.blockProfiles.map((b, index) => b.name)
      // )
      ...
      try {
        await axios
          .post(`${baseUrl}/api/user/api-users`, {
            ...
            gender: user?.gender,
            region: user?.region,
            block: user?.blockProfiles.map((b, index) => b.name),
          })
          .then((res) => {
                    ...
                    <View style={{ marginVertical: 15 }}>
                      <View>
                        <View>
                          ...
                          {blockPeople?.includes(item._id) ? null : (
                            <Pressable
                              onPress={() => {
                                //pushSave(item._id)
                                //saveProfile(item._id)
            ...
            >
              ...
              >
                <View style={{ borderBottomWidth: 1, borderColor: 'gray' }}>
                  ...
                  >
                    {`${blockName}님을 차단하시겠습니까?`}
                  </Text>
                </View>
                <View style={{ marginTop: 10 }}>
                  <Text
                    style={{ color: 'gray' }}
                  >{`${blockName}님을 더이상 추천받지 않습니다`}</Text>
                  <Text style={{ color: 'gray' }}>
                    {`${blockName}님으로부터 하트나 메세지를 받을 수 없습니다`}
                  </Text>
                </View>
